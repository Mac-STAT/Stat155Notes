...
This chapter briefly discusses the theory of formal probability so that we have terminology and basic concepts to understand a random process. This provides a framework for thinking about **uncertainty,** **random variability**,  **behavior in the long run.**
...
```{block type='reflect'}
What is the probability of getting a 1 from a six-sided die? How do you know that?
```

There are three types of probability.

1. **Empirical Probability** If you could repeat the random process over and over again, we'd get a sense of the possible outcomes and their associated probabilities. So if you repeatedly tossed a die, then the relative frequency of 1's over time would be the empirical probability.

2. **Theoretical Probability** If you don't have time to toss a die a million times, you could calculate probabilities based on mathematical theory and assumptions. You could assume that each size is equally likely, thus the chance of getting a 1, is 1/6 for a six-sided die. 

```{block type='reflect'}
What is the probability you'll get an A in this class? What does this number represent?
```

3. **Subjective Probability** If you use a number between 0 and 1 (100%) to reflect your uncertainty, then you are using subjective probability.
...
In theoretical probability, we need to define a few terms and set some rules (axioms).
...
- Example: If you flip two coins, an event $A$ could be that 1 of the coins lands Heads, $A = \{HT,TH\}$.


For the rules of probability, we will define them with set notation as well as words. If you aren't familiar with set notation, 
...
For events $A$ and $B$ and sample space $S$, 
...
Imagine that we have a blood drive in St. Paul. The next donor's blood type can be thought of as a random outcome. The sample space for this random process is $S= \{O,A,B,AB\}$ (all possible outcomes). Assume the people who would volunteer have the same distribution of blood types as the U.S. and that St. Paul has the same distribution as the entire U.S.

Think about how you'd justify your answer:
...
3. P(Type O and Type O and Type O) = $(45/100)^3$ by Rule 5 assuming donors are independent in that the probability of Type O blood stays the same
