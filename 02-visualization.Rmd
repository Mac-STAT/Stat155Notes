...
Now, let's switch to another data set that includes a variety of body measurements. Imagine that you are an entreprenure and you are going to sell dress shirts. Clothing sizing is just weird (don't get me started...) so let's use this dataset to try to come up with our own data driven sizes. Two of the key measurements are the neck size and waist size. There are others, but let's focus on these for the moment.
...
Suppose instead of *Waist* in inches and *Neck size* in inches, we switched to metric and plotted *Waist* and *Neck size* in cm. 

**Does the strength change?**

```{r}
bodyfat %>%
  ggplot(aes(x = Neck*2.54, y = Waist*2.54)) +
  geom_point(color = 'steelblue') + 
  theme_minimal()
```
...
```{r, echo=FALSE}
bodyfat <- bodyfat %>%
  mutate(zNeck = scale(Neck), zWaist = scale(Waist), product = sign(zWaist*zNeck))

bodyfat %>%
  ggplot(aes(x = zNeck, y = zWaist, color = factor(product))) +
  geom_point() +
...
```{r, echo=TRUE}
cor(bodyfat$Neck,bodyfat$Waist)
```
