...
In this section we consider model evaluation: Is this model "good"?
...
The best fitting line is (Predicted College GPA = 1.09 + 0.675*High School GPA). 
...
Below we calculate $SSE$, the sum of squared residuals.
...
Using this model (that is, using your high school GPA), we can predict your college GPA within about $0.56$ GPA points. Is this useful? Is a margin of $0.56$ GPA points good enough?
...
The standard deviation is based on the sum of squared total variation (variation around the mean),
...
We define $SSTO$ because it will help to compare $SSTO$ to $SSE$ to obtain a measure of how well our models are **fitting** the data - how well they are predicting the outcome.
...
```{block type='math'}
$$ R^2 = 1 - \frac{SSE}{SST0} = 1 - \frac{ \sum{(y_i - \hat{y_i})^2}}{ \sum{(y_i - \bar{y})^2}}$$
```
...
We alluded earlier that multiple linear regression could provide estimates of causal effects in the right circumstances. What are those circumstances? In the fireplace-living area interaction model above, we could interpret the -37610 for fireplace presence causally if living area were really the only other confounding variable.

Perhaps you feel let down about this. We could never possibly know of or measure all confounding variables, could we!? This is true, but that doesn't mean that our endeavor to understand causation is fruitless. As long as we can describe the relationship between known confounders as precisely as possible, we have a starting ground for moving forward. We collect data, analyze how well our model predicts that data, and collect more data based on that.
