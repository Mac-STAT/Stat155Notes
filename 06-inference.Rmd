...
Whether by the Central Limit Theorem (mathematical theory) or bootstrapping, confidence intervals provide a *range of plausible values* for the true parameter and allow us to answer both types of questions:
...
The test statistic belo is a reasonable proposal:
...
2. Select $\alpha$, a threshold for what is considered to be a small p-value.
...
$$\text{Test statistic} = \frac{\text{estimate} - \text{null value}}{\text{std. error of estimate}} = \frac{\text{estimate}}{\text{std. error of estimate}} $$
