...
Here is a good summary of why many people strongly dislike pie charts: http://www.businessinsider.com/pie-charts-are-the-worst-2013-6. Keep in mind Visualization Principle #4: Facilitate Comparisons.
...
We can adjust the stacked bar plot to make the heights the same, so that you can compare conditional distributions. For a **stacked bar plot based on proportions**,
...
The best (Prof. H's opinion) graphic for two categorical variables is a variation on the stacked bar plot. The total heights of the bars are the same so we can compare the conditional distributions. For a **mosaic plot**, 
...
The main graphical summary we use for quantitative variables is a histogram. It resembles a bar plot, but there are a few key differences: 
...
If we want to specify the width of the intervals or bins, we add one bit of code:
...
Lastly, notice the y-axis has been the counts (or frequency) within each sleep hour interval. We can adjust this to **density**, which is relative frequency adjusted for the width of interval so that the sum of the areas of the bars (height x width) equals 1. 
...
When verbally describing a distribution, we focus on three aspects of the histogram:
...
```{block, type="reflect"}
Is this distribution of salaries left-skewed or right-skewed? In what populations do you think salaries would be left-skewed? Right-skewed?
```
...
This will not be true always, but it will be true for a particularly special distribution that we will see in the next few chapter when we cover models. (This distribution is called the **normal distribution** or **Gaussian distribution**.)
...
Since shifting (adding or subtracting) and scaling (multiplying or dividing) make no difference in the strength of the relationship, let's standardize both variables into z-scores and replot (and add some color):
...
The blue points in the upper right and lower left quadrants are either both positive or both negative in their z-score values. This means that those individuals are above average in both Neck Size and Chest Size or they are below average in both Neck Size and Chest Size. If we multiply the x and y values for the blue points, we will get a positive value. 
...
An individual far above the means in both or far below the means in both will have a very large, positive product of z-scores. 

An individual far above the means in x and far below the mean in y will have a very large, negative product of z-scores. 
...
require(mvtnorm)
set.seed(123)
d <- data.frame(rmvnorm(100,c(2,3), matrix(c(1,0,0,1),2)))
names(d) <- c('x','y')
d %>%
  ggplot(aes(x=x,y=y)) +
  geom_point() + 
...
d <- d %>% bind_rows(d, data.frame(x = 15, y = 15))
d %>%
  ggplot(aes(x=x,y=y)) +
  geom_point() + 
  theme_minimal() +
  ggtitle('Was: r = 0, Now with Outlier: r = 0.38')


d <- data.frame(rmvnorm(100,c(2,3), matrix(c(1,0.9,0.9,1),2)))
names(d) <- c('x','y')
d %>%
  ggplot(aes(x=x,y=y)) +
  geom_point() + 
...
d <- data.frame(rmvnorm(100,c(2,3), matrix(c(1,-0.6,-0.6,1),2)))
names(d) <- c('x','y')
d %>%
  ggplot(aes(x=x,y=y)) +
  geom_point() + 
