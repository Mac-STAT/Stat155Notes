...
```{block, type="reflect"}
Say that Disease X has a prevalence of 1 in 1,000 (meaning that 1 out of every 1,000 people will have it), and the test to detect it has a false-positive rate of 5 percent (meaning 5 of every 100 subjects test positive for the ailment even though they don’t really have it). If a patient's test result comes back positive, what are the chances that they actually has the disease?
```

If you said the chances are 95%, then you are wrong, but almost half of the doctors surveyed in 2014 thought the exact same thing.

Let's use those rules to get a sense of what the chance is. We want to know the chance they have the disease GIVEN they got a positive test result, $P(D \mid +)$ where $D$ stands for disease and $+$ stands for positive test result.
...
- The false-positive rate is 5 percent, so given you don't have the disease, the chance of getting a false positive is $P(+ \mid\text{ no } D) = 0.05$. So of the 999 that don't have the disease, about $0.05\times 999 = 49.95$ (about 50) of them will get a false positive test result.  
...
$$P(D \mid +) = \frac{P(D \text{ and } +)}{P(+)}= \frac{P( + \mid D) P(D)}{P( + \mid D) P(D) + P( + \mid \text{ no }D) P(\text{no }D)} = \frac{0.99*1/1000}{0.99*1/1000 + 0.05*999/1000} = \frac{0.99*1}{0.99*1 + 0.05*999}$$
...
The concept of conditional probability also plays an important role in the judicial system in the U.S. We assume you are innocent until proven guilty. Thus, juries are instructed to consider the likelihood of the evidence, assuming the person on trial is innocent. In other words, consider the conditional probability: $P(\text{ evidence } \mid \text{ innocent })$.

Unfortunately, many prosecutors try to make the wrong argument. They sometimes mistakenly try and argue that it is unlikely that a person is innocent given the evidence that is available, $P(\text{ innocent } \mid \text{ evidence })$. But this is not what the jury is supposed to consider, thus these types of arguments can lead to retrials.   
...
A **random variable** ($X$) is a real-valued function whose outcome we don't know beforehand. When considering data analysis and modeling, the random variables we will be thinking about will be estimated slopes, estimate odds ratios, etc. To establish our understanding, let's start with a simple example. 
...
Rather, we will use a **named probability model** such that the we can calculate probabilities for particular values of the random variable using either a **probability mass function** (finite number of possible values) or a **probability density function** (infinite number of possible values). 
